...
### 11.3.1 为什么客户端会长时间显示“正在连接代理”

先区分两种时间：

- **建连时间**：当前候选节点从开始 `DialContext` 到拿到可用上游 TCP 通道的时间
- **请求日志耗时**：整条请求/隧道最终持续了多久

从当前版本开始，`SOCKS5` 与 `HTTP CONNECT` 在**建连阶段**会启用快速切换：

- 先尝试 sticky 节点
- 同一出口 IP 最多再尝试 2 个备选节点
- 之后按当前优先层、再按更低优先层继续切换
- fallback tier 最后兜底
...
所以“连接中”停留一段时间并不一定表示 Resin 卡在同一个节点上，也可能是在按顺序快速切换多个候选节点。

如果你要确认是否真的发生了切换，请去 **请求日志详情** 看：

- `切换`
- `尝试次数`
- `尝试链路`

其中会直接展示每次建连尝试的节点、出口 IP、层级、超时、耗时和错误结果。

> 注意：当前这套“当前请求内自动切换”只实现于 `SOCKS5` 与 `HTTP CONNECT`。普通 HTTP 正向代理与反向代理不在已开始发送的请求里中途换节点。
