...
- **业务友好的粘性代理**：让同一业务账号优先绑定同一出口 IP；当 `SOCKS5` / `HTTP CONNECT` 的建连阶段遇到异常节点时，会按“同 IP 优先、再逐层下探”的顺序快速切换候选节点，在多数场景下减少业务波动。
- **双模接入**：同时支持标准正向代理（HTTP Proxy / SOCKS5）与 URL 反向代理（Reverse Proxy）。
- **可观测性**：提供详细的性能指标与日志记录，快速掌控全局（可视化 Web 管理后台）。结构化请求日志支持按平台、账号、目标站点等维度查询与审计；`SOCKS5` / `HTTP CONNECT` 还会额外记录建连切换的尝试链路。
...
### 筛选节点
如果你的服务对节点有筛选要求，例如只需要某个地区的节点，或者只需要来自某个订阅源的节点，或者只需要 `订阅名/tag` 匹配特定正则表达式的节点，可以使用 Resin 的 Platform 概念来实现。
...
如果你希望“包含家宽或住宅，同时排除中转和专线”，可以在平台配置中这样填写：
...
- **账号 (Account)**：业务侧的唯一标识（如 `Tom` 或 `user_1`）。携带特定 Account 的请求，Resin 会优先为其分配稳定的出口节点；当 `SOCKS5` / `HTTP CONNECT` 在建连阶段遇到异常节点时，会优先切换同 IP 节点，再按当前优先层到更低优先层逐层下探，以降低业务侧适配成本。优先级层只影响新建选点，不会打断已有粘性租约，也不会覆盖同 IP 优先切换的顶层规则。

> 当前“当前请求内的快速切换”仅实现于 `SOCKS5` 与 `HTTP CONNECT`。普通 HTTP 正向代理与反向代理仍保持原有语义：请求失败后由后续新请求重新路由，不在已开始发送的请求中途自动换节点。

### CONNECT 建连阶段的快速切换

当客户端通过 `SOCKS5` 或 HTTP 代理的 `CONNECT` 建立隧道时，Resin 会在**建连阶段**启用快速切换：

- 先尝试当前 sticky 节点
- 同一出口 IP 最多再尝试 2 个备选节点
- 之后按当前优先层、再按更低优先层逐层下探
- fallback tier 最后兜底
- 同一节点一条请求内只会尝试一次，不会在同一个坏节点或同一个 IP 组里来回打转
...
这里的“建连超时”指的是：从调用当前候选节点的 `DialContext` 开始，到拿到可用上游 TCP 通道为止。它不是整条业务请求的总耗时，也不是请求日志里隧道保留时长。

### 请求日志中的 CONNECT 尝试链路

当 `SOCKS5` / `HTTP CONNECT` 发生建连切换时，请求日志会额外展示：

- 尝试次数
- 是否发生切换
- 每次尝试的节点、出口 IP、层级、超时、耗时与错误结果

这可以帮助你区分两种情况：

- 只是当前节点拨号慢，但 Resin 已经在持续切换新候选
- 还是实际上一直卡在同一个节点或同一个 IP 组
