...
* RegexFilters：一个正则表达式列表。按照节点的 `订阅名/tag` 的正则表达式做包含过滤。同一个候选标签必须同时满足所有过滤器才符合条件。
* ExcludeRegexFilters：一个正则表达式列表。按照节点的 `订阅名/tag` 的正则表达式做排除过滤。同一个候选标签只要命中任意过滤器就会被排除。
* RegexFiltersCompiled：编译后的包含正则表达式列表。用于运行时匹配。随着 RegexFilters 更新。
...
* platforms(id PK, name UNIQUE, sticky_ttl_ns, regex_filters_json, exclude_regex_filters_json, region_filters_json, reverse_proxy_miss_action, reverse_proxy_empty_account_behavior, reverse_proxy_fixed_account_header, allocation_policy, updated_at_ns)
...
* 必填字段：`name`
* 可选字段：`sticky_ttl`、`regex_filters`、`exclude_regex_filters`、`region_filters`、`reverse_proxy_miss_action`、`reverse_proxy_empty_account_behavior`、`reverse_proxy_fixed_account_header`、`allocation_policy`
...
* `regex_filters`：每项可被 regexp 编译；同一个候选标签需同时满足全部规则。
* `exclude_regex_filters`：每项可被 regexp 编译；同一个候选标签命中任意规则即排除。
...
* 必填字段：无
* 可改字段：`name`、`sticky_ttl`、`regex_filters`、`exclude_regex_filters`、`region_filters`、`reverse_proxy_miss_action`、`reverse_proxy_empty_account_behavior`、`reverse_proxy_fixed_account_header`、`allocation_policy`
...
* `platform_spec` 仅允许字段：`regex_filters`、`exclude_regex_filters`、`region_filters`。

关键校验（最小集）：

* `platform_id`：必须存在。
* `platform_spec.regex_filters`：每项可被 regexp 编译。
* `platform_spec.exclude_regex_filters`：每项可被 regexp 编译。
